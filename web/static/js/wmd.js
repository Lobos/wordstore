...
(function () {
	...
	WMDEditor.defaults = { // {{{
		...
		// entering links.
		imageDialogText: "<p style='margin-top: 0px'><b>输入图片地址.</b></p><p>你可以添加一个标题，鼠标移上去会显示提示信息.</p><p>例:<br />http://i.imgur.com/1cZl4.jpg</p>",
		linkDialogText: "<p style='margin-top: 0px'><b>输入链接地址.</b></p><p>你可以添加一个标题，鼠标移上去会显示提示信息.</p><p>例:<br />http://www.google.com/</p>",
		...
		buttons: "bold italic  link blockquote code image  ol ul heading hr  undo redo preview help",
	...
	var wmdBase = function (wmd, wmd_options) { // {{{
		...
		wmd.editor = function (previewRefreshCallback) { // {{{
			...
			var makeSpritedButtonRow = function () {
				...
				for (var i=0;i<buttonlist.length;i++) {
					switch (buttonlist[i]) {
					case "bold":
						addButton("wmd-bold-button", "粗体 <strong> "+modifierKey+"+B", command.doBold);
						break;
					case "italic":
						addButton("wmd-italic-button", "斜体 <em> "+modifierKey+"+I", command.doItalic);
						break;
					case 'link':
						addButton("wmd-link-button", "链接 <a> "+modifierKey+"+L", function (chunk, postProcessing, useDefaultText) {
							return command.doLinkOrImage(chunk, postProcessing, false);
						});
						break;
					case 'blockquote':
						addButton("wmd-quote-button", "引用 <blockquote> "+modifierKey+"+Q", command.doBlockquote);
						break;
					case 'code':
						addButton("wmd-code-button", "代码 <pre><code> "+modifierKey+"+K", command.doCode);
						break;
					case 'image':
						addButton("wmd-image-button", "图片 <img> "+modifierKey+"+G", function (chunk, postProcessing, useDefaultText) {
							return command.doLinkOrImage(chunk, postProcessing, true);
						});
						break;
					case 'ol':
						addButton("wmd-olist-button", "数字列表 <ol> "+modifierKey+"+O", function (chunk, postProcessing, useDefaultText) {
							command.doList(chunk, postProcessing, true, useDefaultText);
						});
						break;
					case 'ul':
						addButton("wmd-ulist-button", "符号列表 <ul> "+modifierKey+"+U", function (chunk, postProcessing, useDefaultText) {
							command.doList(chunk, postProcessing, false, useDefaultText);
						});
						break;
					case 'heading':
						addButton("wmd-heading-button", "标题 <h1>/<h2> "+modifierKey+"+H", command.doHeading);
						break;
					case 'hr':
						addButton("wmd-hr-button", "水平线 <hr> "+modifierKey+"+R", command.doHorizontalRule);
						break;
					case 'undo':
						var undoButton = addButton("wmd-undo-button", "后退 - "+modifierKey+"+Z");
						undoButton.execute = function (manager) {
							manager.undo();
						};
						break;
					case 'redo':
						var redoButton = addButton("wmd-redo-button", "前进 - "+modifierKey+"+Y");
						if (/win/.test(nav.platform.toLowerCase())) {
							redoButton.title = "前进 - "+modifierKey+"+Y";
						}
						else {
							// mac and other non-Windows platforms
							redoButton.title = "前进 - "+modifierKey+"+Shift+Z";
						}
                    ...
                    case 'preview':
                        ...
                        var previewButton = addButton("wmd-preview-button", '预览');
                        previewButton.onclick = command.doPreview;
                        break;
					case '':
						addSpacer();
						break;
		...
		command.doCode = function (chunk, postProcessing, useDefaultText) {
            ...
            var hasTextBefore = /^[`~]{3,}/.test(chunk.selection);
			var hasTextAfter = /[`~]{3,}$/.test(chunk.selection);

			// Use 'four space' markdown if the selection is on its own
			// line or is multiline.
			if (hasTextAfter && hasTextBefore) {
                var text = chunk.selection;
                text = text.replace(/[`~]{3,}.*\n/gm, '');
		...
		command.doHeading = function (chunk, postProcessing, useDefaultText) {
			...
			// make a level 3 hash header around some default text.
			if (!chunk.selection) {
				chunk.startTag = "### ";
			...
			var headerLevelToCreate = headerLevel == 0 ? 3 : headerLevel;
			...
			if (headerLevelToCreate > 0) {
                ...
                var len = headerLevelToCreate;
                while (len--) {
                    chunk.startTag += '#';
                }
                chunk.startTag += ' ';
                chunk.endTag = ' ';
                len = headerLevelToCreate;
                while (len--) {
                    chunk.endTag += '#';
                }
			}
        ...
        command.doPreview = function () {
            var reg = new RegExp("(^|\\s+)disabled(\\s+|$)");
            if (reg.test(wmd.panels.preview.className)) {
                this.className = this.className.replace(new RegExp("(^|\\s+)disabled(\\s+|$)"), ' ');
                wmd.panels.preview.className = wmd.panels.preview.className.replace(new RegExp("(^|\\s+)disabled(\\s+|$)"), ' ');
            } else {
                this.className += ' disabled';
                wmd.panels.preview.className += ' disabled';
            }
        };
		// }}}
	}; // }}}
