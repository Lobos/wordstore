...
/*
	...
	*/
    ...
    var _DoCodeBlocksTilde = function (text) {
        /*
        ```|~~~{.class}
            code
        ```|~~~
        */
        text = text.replace(/(~T){3,}/mg, function (m1) {
            return '~~~';
        });
        text = text.replace(/[`~]{3,}(?:(.*$)\n)?([\s\S]*)[`~]{3,}/mg, function (wholeMath, m1, m2) {
            var className = m1 || '';
            ...
            codeblock = "<pre><code class='" + className + "'>" + codeblock + "\n</code></pre>";
